...
def dashboard(request, member_id):
    ...
    membership_purchase = MembershipPurchase.objects.filter(
        member=request.user)
    ...
    if membership_purchase:
        ...
        messages.error(
            request, "You must first purchase a membership to \
                access the members dashboard")
...
def edit_private_data(request, member_id):
    ...
    membership_purchase = MembershipPurchase.objects.filter(
        member=request.user)
    member_private = Member_Data_Private.objects.filter(member=request.user)
    member_public_exists = Member_Data_Public.objects.filter(
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(request,
                                 "Member profile changes saved successfully")
...
def edit_public_data(request, member_id):
    ...
    if member_public_exists:
        ...
        if request.method == "POST":
            public_data_form = MembershipPublicDataForm(
                request.POST, instance=member_public)
            if public_data_form.is_valid():
                public_data_form.save()
                messages.success(
                    request,
                    "Your public profile changes saved successfully")
                return redirect('dashboard', member_id)
            else:
                messages.error(
                    request,
...
def delete_public_data(request, member_id):
    """
    Delete Member's Public Profile
    """
    member_public = get_object_or_404(Member_Data_Public, member=member_id)
...
def membership_purchases(request, member_id):
    ...
    membership_purchases = MembershipPurchase.objects.filter(
        member=request.user)
    member_private = Member_Data_Private.objects.filter(member=request.user)
    member_public_exists = Member_Data_Public.objects.filter(
        member=request.user.id)
...
def member_directory(request, member_id):
    ...
    member_profiles = Member_Data_Public.objects.all().order_by(
        'public_lastname').values()
    member_private = Member_Data_Private.objects.filter(member=request.user)
    member_public_exists = Member_Data_Public.objects.filter(
        member=request.user.id)
